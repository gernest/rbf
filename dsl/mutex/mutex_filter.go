...
func (m *MatchString) Apply(txn *tx.Tx, columns *rows.Row) (r *rows.Row, err error) {
	err = txn.Cursor(m.Field, func(c *rbf.Cursor, tx *tx.Tx) error {
		switch m.Op {
		case EQ:
			r, err = m.eq(c, tx, columns)
		case NEQ:
			r, err = m.neq(c, tx, columns)
		case RE:
...
func (m *MatchString) neq(c *rbf.Cursor, tx *tx.Tx, columns *rows.Row) (r *rows.Row, err error) {
	exists, err := m.existence(tx)
	if err != nil {
		return nil, err
	}
	r, err = m.eq(c, tx, columns)
...
func (m *MatchString) existence(txn *tx.Tx) (r *rows.Row, err error) {
...
func (m *MatchString) eq(c *rbf.Cursor, tx *tx.Tx, columns *rows.Row) (r *rows.Row, err error) {
	id, ok := tx.Tr.Find(m.Field, []byte(m.Value))
	if !ok {
		return rows.NewRow(), nil
	}
	return m.eqID(c, tx, id, columns)
}

func (m *MatchString) eqID(c *rbf.Cursor, tx *tx.Tx, id uint64, columns *rows.Row) (r *rows.Row, err error) {
...
func (m *MatchString) re(c *rbf.Cursor, tx *tx.Tx, columns *rows.Row) (r *rows.Row, err error) {
	err = tx.Tr.SearchRe(m.Field, clean(m.Value), nil, nil, func(key []byte, value uint64) error {
		n, err := m.eqID(c, tx, value, columns)
		if err != nil {
			return err
		}
		if r != nil {
